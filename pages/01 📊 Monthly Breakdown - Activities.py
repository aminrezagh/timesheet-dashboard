...
st.markdown('<h1 style="text-align: center;">Monthly Breakdown - Activities</h1><br>', unsafe_allow_html=True)
            ...
            try:
                for n in range(height):
                    ...
                    st.vega_lite_chart(df[df["pers_name"].isin([pers_list[width * n + i]]) &
                     ...
                     ~df["activity_type"].isin(st.session_state["ss_act_range"])].groupby(["date", "activity_type"]).sum().reset_index(), {
                        "width": "container",
                        "height": {"step": 25},
                        "title": {"text": pers_list[width * n + i], "offset": 15, "fontSize": "16", "anchor": "start"},
                        "mark": {"type": "bar", "stroke": "#fff", 
                                "tooltip": {"signal": "{'Total (h)': round(datum.total), 'Activity': datum.activity_type, 'Month': datum.date}"}},
                        "encoding": {
                            "x": {"field": "total", "type": "quantitative", "axis": {"title": "Total (h)"}},
                            "y": {"field": "date", "type": "nominal", "axis": {"title": "Month", "grid": "true"}},
                            "color": {"field": "activity_type", "type": "nominal", "scale": {"domain": dom, "range": rng}}},
                    ...
                    ~df["activity_type"].isin(st.session_state["ss_act_range"])].groupby(["activity_type"]).sum().reset_index(), {
                        ...
                        "mark":{"type": "arc", "innerRadius": 70, "stroke": "#fff", 
                                "tooltip": {"signal": "{'Percent (%)': round(datum.percent), 'Activity': datum.activity_type}"}},
                        "encoding": {
                            "theta": {"field": "percent", "type": "quantitative"},
                            "color": {"field": "activity_type", "type": "quantitative", "type": "nominal", "scale": {"domain": dom, "range": rng}}},
                         "config": {"legend": {"orient": "right", "layout": {"right": {"anchor": "middle"}}}}           
                         }, use_container_width=True)
