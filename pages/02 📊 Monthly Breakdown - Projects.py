...
st.markdown("<h1 style='text-align: center;'>Monthly Breakdown - Projects</h1><br>", unsafe_allow_html=True)
        ...
        with v:
            try:
                for n in range(height):
                    ...
                    st.vega_lite_chart(df[df["pers_name"].isin([pers_list[width * n + i]]) &
                    ~df["date"].isin(st.session_state["ss_month_range"]) & 
                    ~df["activity_type"].isin(st.session_state["ss_act_range"]) & 
                    ~df["cost_center"].isin(st.session_state["ss_prj_range"])].groupby(["date", "cost_center_id", "cost_center"]).sum().reset_index(), {
                        "width": "container",
                        "height": {"step": 25},
                        "title": {"text": pers_list[width * n + i], "offset": 15, "fontSize": "16", "anchor": "start"},
                        "mark": {"type": "bar", "stroke": "#fff", 
                                "tooltip": {"signal": "{'Total (h)': round(datum.total), 'Project': datum.cost_center}"}},
                        "encoding": {
                            "x": {"field": "total", "type": "quantitative", "axis": {"title": "Total (h)"}},
                            "y": {"field": "date", "type": "nominal", "axis": {"title": "Month", "grid": "true"}},
                            "color": {"field": "cost_center_id", "type": "nominal", "scale": {"domain": cc_list, "range": hex_code}}},
                    ...
                    ~df["date"].isin(st.session_state["ss_month_range"]) & 
                    ~df["activity_type"].isin(st.session_state["ss_act_range"]) &
                    ~df["cost_center"].isin(st.session_state["ss_prj_range"])].groupby(["cost_center", "cost_center_id"]).sum().reset_index(), {
                        "transform": [{"joinaggregate": [{"op": "sum", "field": "total", "as": "sumoftotal"}]},
                                    {"calculate": "datum.total/datum.sumoftotal * 100", "as": "percent"}],
                        "width": "container",
                        "height": 400,
                        "title": {"text": f"{pers_list[width * n + i]} - OVERALL BREAKDOWN", "offset": 20, "fontSize": "16", "anchor": "start"},
                        ...
                        "encoding": {
                            ...
                            "color": {"field": "cost_center_id", "type": "quantitative", "type": "nominal", "scale": {"domain": cc_list, "range": hex_code}}},
                         "config": {"legend": {"orient": "right", "layout": {"right": {"anchor": "middle"}}}}           
                         }, use_container_width=True)
