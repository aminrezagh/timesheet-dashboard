...
st.markdown("<h1 style='text-align: center;'>Activity Comparison</h1><br>", unsafe_allow_html=True)
            ...
            try:
                for n in range(height):
                    ...
                    st.vega_lite_chart(df[
                     ...
                     ~df["cost_center"].isin(st.session_state["ss_prj_range"])].groupby(["pers_name", 
                     "activity_type"]).agg(percent=("total", "sum")).groupby(level=0).apply(
                     lambda x: 100 * x / float(x.sum())).reset_index().query('activity_type == @acts[@width * @n + @i]'), {
                        "width": "container",
                        "height": 350,
